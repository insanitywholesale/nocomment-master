...
public final class BlockCheck extends PriorityDispatchable {
	...
	private static final Counter blockCheckCancellations = Counter.build().name("block_check_cancellations_total")
			.help("Number of block check cancellations").labelNames("priority").register();

	private static final Counter blockCheckCancelQueries = Counter.build().name("block_check_cancel_queries_total")
			.help("Number of times a block check cancellation has been queried").labelNames("priority", "outcome")
			.register();
	...
	public void onCompleted(OptionalInt blockState) { // this can be called from any
														// thread at any time
