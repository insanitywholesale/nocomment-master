...
public final class BlockCheckManager {
	...
	private static final Histogram blockPruneLatencies = Histogram.build().name("block_prune_latencies")
			.help("Block prune latencies").labelNames("dimension").register();

	private static final Gauge checkStatusQueueLength = Gauge.build().name("check_status_queue_length")
			.help("Length of the check status queue").register();

	private static final Gauge checkStatusQueueLatency = Gauge.build().name("check_status_queue_latency")
			.help("Age of the head of the check status queue").register();

	private static final Counter checksRan = Counter.build().name("checks_ran_total")
			.help("Number of checks we have run").register();

	private static final Gauge blockCheckStatuses = Gauge.build().name("block_check_statuses_size")
			.help("Size of the block check statuses").labelNames("dimension").register();
	...
	// to avoid overwhelming the main executor with literally thousands of DB queries for
	// blocks and
	// signs from the past
	...
	public static Executor checkStatusExecutor = new LoggingExecutor(
	...
	public BlockCheckManager(World world) {
		this.world = world;
		TrackyTrackyManager.scheduler.scheduleWithFixedDelay(LoggingExecutor.wrap(this::update), 0, 250,
				TimeUnit.MILLISECONDS);
		TrackyTrackyManager.scheduler.scheduleWithFixedDelay(
				LoggingExecutor.wrap(() -> blockPruneLatencies.labels(world.dim()).time(this::blockPrune)),
				PRUNE_INTERVAL, PRUNE_INTERVAL, TimeUnit.MILLISECONDS);
	}
	...
	private void update() {
		...
		try (Connection connection = Database.getConnection();
				PreparedStatement stmt = connection.prepareStatement(
						"INSERT INTO blocks (x, y, z, block_state, created_at, dimension, server_id) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
	...
	public enum BlockEventType {
		...
		UNLOADED, FIRST_TIME, MATCHES_PREV, UPDATED, CACHED
	...
	private synchronized BlockCheckStatus get(long bpos) {
		return statuses.computeIfAbsent(BlockPos.blockToChunk(bpos), cpos -> new Long2ObjectOpenHashMap<>())
				.computeIfAbsent(bpos, BlockCheckStatus::new);
	}
	...
	private synchronized void unloadedAt(long cpos, long now) {
		...
		unloadObservationExecutor
				.execute(() -> s.forEach(status -> status.onResponseInternal(OptionalInt.empty(), now)));
	}
	...
	private synchronized void blockPrune() {
		synchronized (pruneLock) {
			...
			ObjectIterator<Long2ObjectMap.Entry<Long2ObjectOpenHashMap<BlockCheckStatus>>> outerIt = statuses
					.long2ObjectEntrySet().fastIterator();
			while (outerIt.hasNext()) {
				Long2ObjectMap.Entry<Long2ObjectOpenHashMap<BlockCheckStatus>> outerEntry = outerIt.next();
				ObjectIterator<Long2ObjectMap.Entry<BlockCheckStatus>> innerIt = outerEntry.getValue()
						.long2ObjectEntrySet().fastIterator();
			...
			Map<Integer, Long> countByPriority = statuses.values().stream().map(Map::values).flatMap(Collection::stream)
					.collect(Collectors.groupingBy(bcs -> bcs.highestSubmittedPriority, Collectors.counting()));
			System.out.println(
					"Cache size change should be " + numActuallyRemoved + " but was actually " + (beforeSz - afterSz));
			blockCheckStatuses.labels(world.dim()).set(afterSz);
			System.out.println("FASTER? Block prune in block check manager took " + (System.currentTimeMillis() - now)
					+ "ms. Cache size went from " + beforeSz + " to " + afterSz + ". Maybe but not actually: "
					+ maybeButNotActually + ". Count by priority: " + countByPriority);
		}
	}
	...
	public void requestBlockState(long mustBeNewerThan, BlockPos pos, int priority, BlockListener onCompleted) {
		...
		NoComment.executor.execute(() -> {
			synchronized (BlockCheckManager.this) { // i hate myself
				synchronized (pruneLock) {
					get(bpos).requested(mustBeNewerThan, priority, onCompleted); // this
																					// is
																					// fine
																					// since
																					// requested
	...
	private static void checkStatusConsumer() {
		...
		try (Connection connection = Database.getConnection();
				PreparedStatement stmt = connection.prepareStatement(
						"SELECT block_state, created_at FROM blocks WHERE x = ? AND y = ? AND z = ? AND dimension = ? AND server_id = ? ORDER BY created_at DESC LIMIT 1")) {
			connection.setAutoCommit(false);
	...
	public class BlockCheckStatus {
		...
		// split up these two lists into "first" and "rest" to optimize RAM for the common
		// case which
		// are only 0 or 1 entries
		...
		private boolean blockStateOptionalPresent; // this optional is "destructured" into
													// an int + a boolean for
													// the same reason - there are tens of
													// millions of these on the
													// heap, and every little thing counts

		private long responseAt;

		private CompletableFuture<Boolean> checkedDatabaseYet; // IMPORTANT: this field is
																// overwritten after
																// completion to
																// STATIC_DATABASE_CHECKED_COMPLETION
		...
		private boolean actuallyPrunable(long now) {
			...
			return maybePrunable(now) && firstListener == null && otherListeners == null && firstInFlight == null
					&& otherInFlight == null;
		}

		private synchronized void checkDatabase(PreparedStatement stmt) throws SQLException { // this
																								// synchronized
																								// is
																								// just
																								// for
			...
			try {
				...
				// it's also okay if .thenAcceptAsync is called right here and races
				// between these two lines
				// - the old checkedDatabaseYet will instantly dispatch since it was
				// previously completed
				checkedDatabaseYet = STATIC_DATABASE_CHECKED_COMPLETION;
				// from now on, checkedDatabaseYet just needs to be an insta-dispatching
				// completed Future
		...
		public void requested(long mustBeNewerThan, int priority, BlockListener listener) {
			...
			checkedDatabaseYet.thenAcceptAsync(ignored -> requested0(mustBeNewerThan, priority, listener),
					NoComment.executor);
		}
		...
		private synchronized void requested0(long mustBeNewerThan, int priority, BlockListener listener) {
			...
			if (unloadedAt != -1 && unloadedAt > mustBeNewerThan
					&& !(blockStateOptionalPresent && responseAt > unloadedAt)) {
				// if this is unloaded, since the newer than, and not older than a real
				// response
				// then that's what we do
				onResponseInternal(OptionalInt.empty(), unloadedAt);
				NoComment.executor
						.execute(() -> listener.accept(OptionalInt.empty(), BlockEventType.UNLOADED, unloadedAt));
				return;
			}
