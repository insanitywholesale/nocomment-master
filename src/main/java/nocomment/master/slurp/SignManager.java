...
public final class SignManager {
	...
	public void request(long mustBeNewerThan, BlockPos pos, Consumer<Optional<byte[]>> consumer) {
		try (Connection connection = Database.getConnection();
				PreparedStatement stmt = connection.prepareStatement(
						"SELECT nbt FROM signs WHERE x = ? AND y = ? AND z = ? AND dimension = ? AND server_id = ? AND created_at > ? ORDER BY created_at DESC LIMIT 1")) {
			stmt.setInt(1, pos.x);
	...
	public synchronized void response(BlockPos pos, Optional<byte[]> data) {
		...
		// this can use main executor since sign RESPONSES (unlike sign requests) will
		// occur at max once
		// a second per bot
	...
	private void saveToDB(BlockPos pos, byte[] data) {
		try (Connection connection = Database.getConnection();
				PreparedStatement stmt = connection.prepareStatement(
						"INSERT INTO signs (x, y, z, nbt, created_at, dimension, server_id) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
			stmt.setInt(1, pos.x);
			stmt.setShort(2, (short) pos.y);
			stmt.setInt(3, pos.z);
