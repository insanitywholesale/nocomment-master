...
public final class ChunkManager {
	...
	private static final Gauge chunkCache = Gauge.build().name("chunk_manager_cache")
			.help("Number of chunk positions by state").labelNames("state").register();

	private static final Counter chunkRequests = Counter.build().name("chunk_manager_requests_total")
			.help("Number of chunks requested").register();

	private static final Counter chunkResponses = Counter.build().name("chunk_manager_responses_total")
			.help("Number of chunks received").register();

	private static final int MAX_SIZE = 2048; // about 500MB RAM

	private final Long2LongOpenHashMap lastAccessed = new Long2LongOpenHashMap(); // ChunkPos,
																					// Time
	...
	public synchronized CompletableFuture<int[]> getChunk(long cpos) {
		...
		return cache.computeIfAbsent(cpos, $ -> {
			...
			CompletableFuture<int[]> ret = new CompletableFuture<>(); // this is OK since
																		// we hold the
																		// lock until
																		// after it's
																		// inserted
	...
	private void fetchLoop() throws IOException, InterruptedException {
		Socket s = new Socket(Config.getRuntimeVariable("GENERATOR_HOST", "localhost"),
				Integer.parseInt(Config.getRuntimeVariable("GENERATOR_PORT", "5021")));
		DataInputStream in = new DataInputStream(new BufferedInputStream(s.getInputStream()));
		DataOutputStream out = new DataOutputStream(s.getOutputStream());
		while (true) {
			while (queue.isEmpty()) {
				// there is no blocking wait to wait until a queue is nonempty, without
				// actually popping the
				// head
			...
			// System.out.println("Cache map size is " + cache.size() + " and total age is
			// " + num);
			chunkResponses.inc();
			synchronized (this) {
				cache.get(cpos).complete(ret);
				while (cache.size() - queue.size() > MAX_SIZE) { // obv can't use
																	// cache.size
					Optional<Long2ObjectMap.Entry<CompletableFuture<int[]>>> toPrune = cache.long2ObjectEntrySet()
							.stream().filter(entry -> entry.getValue().isDone())
