...
public final class ClusterRetryScanner {
	...
	public ClusterRetryScanner(World world, int priority, int volume, long rerunDelayMS, Consumer<Hit> onHit) {
		System.out.println(
				"Constructing cluster retry scanner with priority " + priority + " and rerun delay " + rerunDelayMS);
		this.world = world;
	...
	public void submitTasks() {
		for (int i = 0; i < volume; i++) {
			schedule(); // don't run the cluster traversal algorithm on the "main" thread
						// for this
						// connection, blocking server construction
		}
	}

	private void schedule() {
		TrackyTrackyManager.scheduler.schedule(LoggingExecutor.wrap(this::submitTask), rerunDelayMS,
	...
	private void submitTask() {
		...
		if (!pos.isPresent()) {
			...
			TrackyTrackyManager.scheduler.schedule(LoggingExecutor.wrap(this::submitTask), rerunDelayMS * 100,
					TimeUnit.MILLISECONDS);
			return;
		}
