...
public final class Staggerer {
	...
	private static final Histogram staggererLatencies = Histogram.build().name("staggerer_latencies")
			.help("Staggerer latencies").register();

	private static final Counter kicks = Counter.build().name("stagerer_kicks").help("Staggerer kicks")
			.labelNames("dimension", "server", "username").register();
	...
	public static void beginStaggerer2b2tPreset(World world) {
		beginStaggerer(world, new int[] { 167548, 132678 }, // 100010, liejurv
	...
	private List<QueueStat> getInQueue() {
		try (java.sql.Connection connection = Database.getConnection();
				PreparedStatement stmt = connection.prepareStatement(
						"SELECT data, player_id, updated_at FROM statuses WHERE curr_status = 'QUEUE'::statuses_enum AND dimension = ?")) {
	...
	private void run() {
		...
		if (world.server.hostname.equals("2b2t.org")) { // i mean this is sorta rart idk
			for (QueueStat qs : getInQueue()) {
				...
				// this is safe since statuses will be reset from QUEUE to OFFLINE within
				// 60s of kick
				// and getInQueue will only return statuses that are currently QUEUE
				observedAt.merge(qs.playerID, qs.when, Math::max);
				playerJoinTS.put(qs.playerID,
						qs.when + qs.pos * QueueStatus.getEstimatedMillisecondsPerQueuePosition());
			}
		...
		for (Connection conn : onlineNow) {
			...
			if (!join.isPresent()) {
				System.out.println(
						"Exceptional situation (i put that in so it would show up when i grep for Exception in the server logs) where a connection is here but not actually on the server "
								+ System.currentTimeMillis() + " " + conn + " " + conn.getIdentity());
				return;
			}
		...
		if (harem.stream().anyMatch(id -> !playerJoinTS.containsKey(id))) {
			...
			// (this means that we're in a transient state awaiting a server restart or
			// reconnect)
		...
		for (int pid : harem.stream().sorted(Comparator.comparingLong(pid -> -leaveAt(playerJoinTS.get(pid), pid, now)))
			...
			if (prevLeaveAt != null) {
				...
				// it just depends on this for loop being in sorted order from youngest to
				// oldest (aka: last
				// connection first, first connection last)
			...
			System.out.println("Player " + pid + " joined at " + format(joinAt) + " would be kicked at "
					+ format(serverLeaveAt) + " but we'll kick at " + format(ourLeaveAt));
			/*
			 * long missedTime = serverLeaveAt - ourLeaveAt; if (missedTime >
			 * TimeUnit.MINUTES.toMillis(121)) { leaveAtTS.remove(pid); }
			 */
		}
		for (Connection conn : onlineNow) {
	...
	private static long leaveAt(long joinTS, int pid, long now) {
		 ...
		 * jewishbanker oremongoloid xz_9 p7k leave += CRAP_KICK; } else {
		 */
		leave += AUTO_KICK;
		// }
