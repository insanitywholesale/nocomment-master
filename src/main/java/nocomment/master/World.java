...
public final class World {
	...
	private static final Gauge worldQueueLength = Gauge.build().name("world_queue_length")
			.help("Length of the world queue").labelNames("dimension", "priority").register();

	private static final Gauge worldQueueSize = Gauge.build().name("world_queue_size")
			.help("Size of the world queue (including tasks with large counts)").labelNames("dimension", "priority")
			.register();
	...
	public World(Server server, short dimension) {
		...
		TrackyTrackyManager.scheduler.scheduleAtFixedRate(LoggingExecutor.wrap(() -> {
			synchronized (World.this) {
				priorityCountsOnHeap
						.forEach((priority, count) -> worldQueueLength.labels(dim(), priority + "").set(count));
	...
	public synchronized void incomingConnection(Connection connection) {
		System.out.println("Connection identified as UUID " + connection.getUUID() + " which is player database ID "
	...
	public synchronized void connectionClosed(Connection conn) {
		// due to the read loop structure, by the time we get here we know for a fact that
		// this
		// connection will read no further data, since its read loop is done
		// so, shuffling the tasks elsewhere is safe, and doesn't risk "completed" being
		// called twice or
		// anything like that
	...
	public synchronized void chunkChecksLookup(Iterator<Long> cposItr,
			BiConsumer<Long, Integer> withPendingBlockChecks) {
	...
	private synchronized void sendTasksOnConnections() {
		...
		while (!heap.isEmpty()) {
			...
			if (conn == null) {
				break; // can't send anything rn, burden too high on all conns.
						// backpressure time!
			}
	...
	public synchronized void submitSign(BlockPos pos) {
		if (connections.isEmpty()) {
			// can't do signManager.response while holding synchronized(World) because
			// SignManager also
			// synchronizes on a database access, in request, and we don't want World to
			// block on db
			NoComment.executor.execute(() -> signManager.response(pos, Optional.empty()));
			return;
		}
		// must hold synchronized so that we don't race with connectionClosed for sign
		// redistributing
	...
	public void worldUpdate() { // something has changed (a connection has completed a
								// task). time to get a
								// new one
	...
	public void notifyHit(ChunkPos pos, int prio) {
		NoComment.executor.execute(() -> {
			...
			if (slurpManager != null) {
				slurpManager.arbitraryHitExternal(pos); // notify slurper that a filter
														// has hit this chunk
			}
		});
	}
