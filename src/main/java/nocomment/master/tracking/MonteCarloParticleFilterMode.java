...
public class MonteCarloParticleFilterMode extends AbstractFilterMode {
	...
	private static final Counter failures = Counter.build().name("monte_carlo_failures_total")
			.help("Steps with no hits nor misses").register();

	private static final Counter timeouts = Counter.build().name("monte_carlo_timeouts_total")
			.help("Filters killed for 120 seconds of no responses").register();

	private static final Counter noGuesses = Counter.build().name("monte_carlo_no_guesses_total")
			.help("Filters killed for no guesses").register();

	private static final Counter missesCtr = Counter.build().name("monte_carlo_misses_total")
			.help("Steps with no hits, just misses").register();
	...
	public static boolean wouldLoadChk(double x, double z, ChunkPos pos, int renderDistance) {
		return Math.abs((int) Math.floor(x) - pos.x) <= renderDistance
				&& Math.abs((int) Math.floor(z) - pos.z) <= renderDistance;
	}
	...
	public MonteCarloParticleFilterMode(ChunkPos start, Track parent) {
		...
		if (GUI) {
			frame = setupFrame();
			TrackyTrackyManager.scheduler.scheduleAtFixedRate(LoggingExecutor.wrap(frame::repaint), 0, 100,
	...
	public List<ChunkPos> updateStep(List<ChunkPos> hits, List<ChunkPos> misses) {
		if (hits.isEmpty() && misses.isEmpty()) {
			...
			if (iterationsWithoutAnything++ > 120) {
				...
				// the bot itself going offline then coming back online will resume the
				// paused filters
				return null;
			}
	...
	private void applyRnd(ParticleList newer, double dt, MutableParticle older) {
		double x = older.x + dt * random.nextGaussian() * older.dx / 15; // also tuned by
																			// trial and
																			// error
