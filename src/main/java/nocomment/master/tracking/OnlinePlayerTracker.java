...
public class OnlinePlayerTracker {
	...
	private static final Gauge onlinePlayers = Gauge.build().name("online_players").help("Number of online players")
			.labelNames("server").register();
	...
	public synchronized void update() {
		...
		if (onlinePlayerSet.isEmpty()) {
			try {
				// when a player joins for the first time, they send us all their online
				// players all at once
				// this, in practice, ends up being exactly two batches in this functions,
				// since the
				// function takes some time the first time, so that by the second time
				// they're all in
		...
		if (!toAdd.isEmpty()) {
			...
			Set<Integer> trackIDs = Database.trackIDsToResume(toAdd, server.serverID); // have
																						// to
																						// do
																						// this
																						// on
	...
	private Set<OnlinePlayer> coalesceFromConnections() {
		...
		// while this is thread safe, the connections can change at any time. collect them
		// into one list
		// just for these two consecutive for loops. god i'm paranoid!
	...
	private void clearQuelled() {
		for (OnlinePlayer player : new ArrayList<>(removalTimestamps.keySet())) { // god i
																					// hate
	...
	// the actual list is a UNION of all active connections lists, which are separately
	// maintained by
	// each connection
	// with an additional "stickiness" clause, which is that a "remove player" also quells
	// any
	// connection contributing that player to the pot, for the next 30 seconds
	...
	// bot B has been connected to the server, and bot B forwards this player_remove event
	// immediately
	...
	// bot A's connection to master drops, and the "X disconnected" message never goes
	// through
